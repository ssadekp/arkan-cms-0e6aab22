      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {partners.map((p: any) => (
          <a key={p.id} href={p.website_url ?? "#"} target="_blank" rel="noreferrer"
             className="flex flex-col items-center gap-3 p-4 rounded-xl border border-border/60 bg-card hover:border-primary/60 transition">
            {p.logo_url ? <img src={p.logo_url} alt={nameOf(p)} className="h-16 w-full object-contain" /> : <div className="h-16 w-full rounded bg-muted" />}
