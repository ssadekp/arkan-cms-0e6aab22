...
function Body() {
  ...
  const others = ((data as any).allFocus ?? []).map((f: any) => ({
    key: f.slug,
    label:
      ...
      <div className="container-narrow py-12 grid gap-10 lg:grid-cols-[minmax(0,1fr)_310px] items-start">
        <div className="min-w-0">
          ...
          {data.projects.length > 0 && (
            <div className="mt-16">
              <h2 className="text-2xl font-semibold mb-6">{lang === "ar" ? "المشروعات" : "Projects"}</h2>
        ...
        <aside className="space-y-6 lg:sticky lg:top-24">
          <SidebarCard title={lang === "ar" ? "مجالات العمل الأخرى" : "Other focus areas"}>
            <SidebarList items={others} to="/focus-areas/$slug" />
          </SidebarCard>
        </aside>
      </div>
    </article>
