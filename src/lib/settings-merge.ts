...
/** Merge for the Homepage admin: patch hero_* fields, preserve everything else. */
export function mergeHomepageI18n(
  lang: Lang,
  existing: Partial<SettingsI18nRow> | null | undefined,
  patch: Partial<Pick<SettingsI18nRow, (typeof HERO_KEYS)[number]>>,
): SettingsI18nRow {
  const merged = base(existing);
  for (const k of HERO_KEYS) merged[k] = patch[k] ?? merged[k];
  return { lang, ...merged };
}
